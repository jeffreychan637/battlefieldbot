...
void PWM_G_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        PWM_G_CONTROL_REG = PWM_G_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        PWM_G_TRIG_CONTROL1_REG  = PWM_G_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        PWM_G_SetInterruptMode(PWM_G_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        PWM_G_SetCounterMode(PWM_G_COUNT_DOWN);
        PWM_G_WritePeriod(PWM_G_QUAD_PERIOD_INIT_VALUE);
        PWM_G_WriteCounter(PWM_G_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (PWM_G__QUAD == PWM_G_CONFIG) */

    #if (PWM_G__TIMER == PWM_G_CONFIG)
        PWM_G_CONTROL_REG = PWM_G_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        PWM_G_TRIG_CONTROL1_REG  = PWM_G_TIMER_SIGNALS_MODES;
        ...
        #if (PWM_G_CY_TCPWM_V2 && PWM_G_TIMER_UPDOWN_CNT_USED && !PWM_G_CY_TCPWM_4000)
            PWM_G_WriteCounter(1u);
        #elif(PWM_G__COUNT_DOWN == PWM_G_TC_COUNTER_MODE)
            PWM_G_WriteCounter(PWM_G_TC_PERIOD_VALUE);
        #else
            PWM_G_WriteCounter(0u);
        #endif /* (PWM_G_CY_TCPWM_V2 && PWM_G_TIMER_UPDOWN_CNT_USED && !PWM_G_CY_TCPWM_4000) */
    #endif  /* (PWM_G__TIMER == PWM_G_CONFIG) */

    #if (PWM_G__PWM_SEL == PWM_G_CONFIG)
        PWM_G_CONTROL_REG = PWM_G_CTRL_PWM_BASE_CONFIG;

        #if (PWM_G__PWM_PR == PWM_G_PWM_MODE)
            PWM_G_CONTROL_REG |= PWM_G_CTRL_PWM_RUN_MODE;
            PWM_G_WriteCounter(PWM_G_PWM_PR_INIT_VALUE);
        #else
            PWM_G_CONTROL_REG |= PWM_G_CTRL_PWM_ALIGN | PWM_G_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (PWM_G_CY_TCPWM_V2 && PWM_G_PWM_UPDOWN_CNT_USED && !PWM_G_CY_TCPWM_4000)
                PWM_G_WriteCounter(1u);
            #elif (PWM_G__RIGHT == PWM_G_PWM_ALIGN)
                PWM_G_WriteCounter(PWM_G_PWM_PERIOD_VALUE);
            #else 
                PWM_G_WriteCounter(0u);
            #endif  /* (PWM_G_CY_TCPWM_V2 && PWM_G_PWM_UPDOWN_CNT_USED && !PWM_G_CY_TCPWM_4000) */
        #endif  /* (PWM_G__PWM_PR == PWM_G_PWM_MODE) */

        #if (PWM_G__PWM_DT == PWM_G_PWM_MODE)
            PWM_G_CONTROL_REG |= PWM_G_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (PWM_G__PWM_DT == PWM_G_PWM_MODE) */

        #if (PWM_G__PWM == PWM_G_PWM_MODE)
            PWM_G_CONTROL_REG |= PWM_G_CTRL_PWM_PRESCALER;
        #endif  /* (PWM_G__PWM == PWM_G_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        PWM_G_TRIG_CONTROL1_REG  = PWM_G_PWM_SIGNALS_MODES;
...
void PWM_G_Enable(void)
{
    ...
    #if (PWM_G__QUAD == PWM_G_CONFIG)
        #if (0u != PWM_G_QUAD_AUTO_START)
            PWM_G_TriggerCommand(PWM_G_MASK, PWM_G_CMD_RELOAD);
        #endif /* (0u != PWM_G_QUAD_AUTO_START) */
    #endif  /* (PWM_G__QUAD == PWM_G_CONFIG) */
}
...
*
* Summary:
*  Initializes the PWM_G with default customizer
...
*  calls to the PWM_G_Start() routine.
*
*******************************************************************************/
...
*  PWM_G operates in the continuous mode.
*
* Parameters:
*  oneShotEnable
*   Values:
...
{
    #if (PWM_G_CY_TCPWM_4000)
        uint32 currentMode;
    #endif /* (PWM_G_CY_TCPWM_4000) */

    #if (PWM_G_CY_TCPWM_4000)
        currentMode = ((PWM_G_CONTROL_REG & PWM_G_UPDOWN_MASK) >> PWM_G_UPDOWN_SHIFT);

        if (((uint32)PWM_G__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)PWM_G__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 PWM_G_ReadCompare(void)
{
        ...
        regVal = PWM_G_COMP_CAP_REG;
        
        if (((uint32)PWM_G__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)PWM_G__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & PWM_G_16BIT_MASK);
    #else
...
void PWM_G_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)PWM_G__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)PWM_G__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
*   Values:
*     - PWM_G_CMD_CAPTURE    - Trigger Capture/Switch command
*     - PWM_G_CMD_RELOAD     - Trigger Reload/Index command
*     - PWM_G_CMD_STOP       - Trigger Stop/Kill command
*     - PWM_G_CMD_START      - Trigger Start/phiB command
*
* Return:
*  None
*
