...
void PWM_R_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        PWM_R_CONTROL_REG = PWM_R_CTRL_QUAD_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        PWM_R_TRIG_CONTROL1_REG  = PWM_R_QUAD_SIGNALS_MODES;

        /* Set values from customizer to INTR */
        PWM_R_SetInterruptMode(PWM_R_QUAD_INTERRUPT_MASK);
        
         /* Set other values */
        PWM_R_SetCounterMode(PWM_R_COUNT_DOWN);
        PWM_R_WritePeriod(PWM_R_QUAD_PERIOD_INIT_VALUE);
        PWM_R_WriteCounter(PWM_R_QUAD_PERIOD_INIT_VALUE);
    #endif  /* (PWM_R__QUAD == PWM_R_CONFIG) */

    #if (PWM_R__TIMER == PWM_R_CONFIG)
        PWM_R_CONTROL_REG = PWM_R_CTRL_TIMER_BASE_CONFIG;
        
        /* Set values from customizer to CTRL1 */
        PWM_R_TRIG_CONTROL1_REG  = PWM_R_TIMER_SIGNALS_MODES;
        ...
        #if (PWM_R_CY_TCPWM_V2 && PWM_R_TIMER_UPDOWN_CNT_USED && !PWM_R_CY_TCPWM_4000)
            PWM_R_WriteCounter(1u);
        #elif(PWM_R__COUNT_DOWN == PWM_R_TC_COUNTER_MODE)
            PWM_R_WriteCounter(PWM_R_TC_PERIOD_VALUE);
        #else
            PWM_R_WriteCounter(0u);
        #endif /* (PWM_R_CY_TCPWM_V2 && PWM_R_TIMER_UPDOWN_CNT_USED && !PWM_R_CY_TCPWM_4000) */
    #endif  /* (PWM_R__TIMER == PWM_R_CONFIG) */

    #if (PWM_R__PWM_SEL == PWM_R_CONFIG)
        PWM_R_CONTROL_REG = PWM_R_CTRL_PWM_BASE_CONFIG;

        #if (PWM_R__PWM_PR == PWM_R_PWM_MODE)
            PWM_R_CONTROL_REG |= PWM_R_CTRL_PWM_RUN_MODE;
            PWM_R_WriteCounter(PWM_R_PWM_PR_INIT_VALUE);
        #else
            PWM_R_CONTROL_REG |= PWM_R_CTRL_PWM_ALIGN | PWM_R_CTRL_PWM_KILL_EVENT;
            
            /* Initialize counter value */
            #if (PWM_R_CY_TCPWM_V2 && PWM_R_PWM_UPDOWN_CNT_USED && !PWM_R_CY_TCPWM_4000)
                PWM_R_WriteCounter(1u);
            #elif (PWM_R__RIGHT == PWM_R_PWM_ALIGN)
                PWM_R_WriteCounter(PWM_R_PWM_PERIOD_VALUE);
            #else 
                PWM_R_WriteCounter(0u);
            #endif  /* (PWM_R_CY_TCPWM_V2 && PWM_R_PWM_UPDOWN_CNT_USED && !PWM_R_CY_TCPWM_4000) */
        #endif  /* (PWM_R__PWM_PR == PWM_R_PWM_MODE) */

        #if (PWM_R__PWM_DT == PWM_R_PWM_MODE)
            PWM_R_CONTROL_REG |= PWM_R_CTRL_PWM_DEAD_TIME_CYCLE;
        #endif  /* (PWM_R__PWM_DT == PWM_R_PWM_MODE) */

        #if (PWM_R__PWM == PWM_R_PWM_MODE)
            PWM_R_CONTROL_REG |= PWM_R_CTRL_PWM_PRESCALER;
        #endif  /* (PWM_R__PWM == PWM_R_PWM_MODE) */

        /* Set values from customizer to CTRL1 */
        PWM_R_TRIG_CONTROL1_REG  = PWM_R_PWM_SIGNALS_MODES;
...
void PWM_R_Enable(void)
{
    ...
    #if (PWM_R__QUAD == PWM_R_CONFIG)
        #if (0u != PWM_R_QUAD_AUTO_START)
            PWM_R_TriggerCommand(PWM_R_MASK, PWM_R_CMD_RELOAD);
        #endif /* (0u != PWM_R_QUAD_AUTO_START) */
    #endif  /* (PWM_R__QUAD == PWM_R_CONFIG) */
}
...
*
* Summary:
*  Initializes the PWM_R with default customizer
...
*  calls to the PWM_R_Start() routine.
*
*******************************************************************************/
...
*  PWM_R operates in the continuous mode.
*
* Parameters:
*  oneShotEnable
*   Values:
...
{
    #if (PWM_R_CY_TCPWM_4000)
        uint32 currentMode;
    #endif /* (PWM_R_CY_TCPWM_4000) */

    #if (PWM_R_CY_TCPWM_4000)
        currentMode = ((PWM_R_CONTROL_REG & PWM_R_UPDOWN_MASK) >> PWM_R_UPDOWN_SHIFT);

        if (((uint32)PWM_R__COUNT_DOWN == currentMode) && (0xFFFFu != compare))
        {
            compare++;
        }
        else if (((uint32)PWM_R__COUNT_UP == currentMode) && (0u != compare))
        {
            compare--;
        }
        else
...
uint32 PWM_R_ReadCompare(void)
{
        ...
        regVal = PWM_R_COMP_CAP_REG;
        
        if (((uint32)PWM_R__COUNT_DOWN == currentMode) && (0u != regVal))
        {
            regVal--;
        }
        else if (((uint32)PWM_R__COUNT_UP == currentMode) && (0xFFFFu != regVal))
        {
            regVal++;
        }
        else
        ...
        return (regVal & PWM_R_16BIT_MASK);
    #else
...
void PWM_R_WriteCompareBuf(uint32 compareBuf)
{
        ...
        if (((uint32)PWM_R__COUNT_DOWN == currentMode) && (0xFFFFu != compareBuf))
        {
            compareBuf++;
        }
        else if (((uint32)PWM_R__COUNT_UP == currentMode) && (0u != compareBuf))
        {
            compareBuf --;
        }
        else
...
*   Values:
*     - PWM_R_CMD_CAPTURE    - Trigger Capture/Switch command
*     - PWM_R_CMD_RELOAD     - Trigger Reload/Index command
*     - PWM_R_CMD_STOP       - Trigger Stop/Kill command
*     - PWM_R_CMD_START      - Trigger Start/phiB command
*
* Return:
*  None
*
